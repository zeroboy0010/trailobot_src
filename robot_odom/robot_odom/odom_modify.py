...
class odometry(Node):
    ...
    def listener_callback(self, init_msg):
        self.position = [init_msg.pose.pose.position.x, init_msg.pose.pose.position.y, init_msg.pose.pose.position.z]
        self.q = [init_msg.pose.pose.orientation.x, init_msg.pose.pose.orientation.y, init_msg.pose.pose.orientation.z, init_msg.pose.pose.orientation.w]
        self.position[2] = euler_from_quaternion(self.q[0], self.q[1], self.q[2], self.q[3])
        print(self.position)
        print(self.q)
        print("Initial Pose")
